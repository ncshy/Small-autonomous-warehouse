...
byte Mrf24j::read_short(byte address) {
    ...
    spi_write(fd, address<<1 & 0b01111110);
    byte ret = spi_write(fd, 0x00);
   // usleep(400);
    return ret;
}
...
byte Mrf24j::read_long(word address) {
    ...
    byte alow = address << 5;
    spi_write(fd, 0x80 | ahigh);  // high bit for long
    spi_write(fd, alow);
    byte ret = spi_write(fd, 0x00);
   // usleep(400);
    return ret;
...
void Mrf24j::write_short(byte address, byte data) {
    ...
    spi_write(fd, (address<<1 & 0b01111110) | 0x01);
    spi_write(fd, data);
}
...
void Mrf24j::write_long(word address, byte data) {
    ...
    byte alow = address << 5;
    spi_write(fd, 0x80 | ahigh);  // high bit for long
    spi_write(fd, alow | 0x10);  // last bit for write
    spi_write(fd, data);
}
...
void Mrf24j::set_pan(word panid) {
    ...
    printf("MRF_PANIDH: %08x\n ", read_short(MRF_PANIDH)); // – Initialize FIFOEN = 1 and TXONTS = 0x6.
    printf("MRF_PANIDL: %08x\n ", read_short(MRF_PANIDL)); // – Initialize FIFOEN = 1 and TXONTS = 0x6.
...
void Mrf24j::init(void) {
    ...
    */
    ...
    usleep(400); // delay at least 192usec
}
